...
if (isset($_POST['submit'])) 
{
  ...
  if(empty($_POST['roll']))
  {
      $error["roll"]='Answer of Security Question is required';
      $noe=$noe+1;
  } 
  else
  {
  ...
  <div class="main-bg" style="text-align: center;padding-bottom: 50px;">
    ...
    <div class="container text-center dibba" style="background-color:#ebbb38;border-radius: 25px;padding-bottom: 50px;">
      ...
      <div class="row" style="padding: 20px;">
        <a href="adminl.php">
        <div class="col-sm-6 text-center tabl inactive slidel">
          Login
        </div>
        </a>
        
        <div class="col-sm-6 text-center tabr  slider">
          Forgot Password
        </div>
      </div>
      <?php
        if($noe===0&&$sub===1)
        {
        $sql="SELECT * FROM admins WHERE name='$name' AND roll='$roll' AND email='$email' AND ques='$ques'";
        ...
        if(mysqli_num_rows($res)>0)
        {
          ...
          <div class="alert alert-success" role="alert" style="width: 100%; align-self: center;border-radius: 20px;border: 1px solid maroon;">
            <?php echo "Password of your account is <b>".$row["password"]."</b>";?>  
          </div>
          </div>
          <?php 
        }
        else
        {
          ...
          <div class="alert alert-danger " role="alert" style="width: 100%; align-self: center;border-radius: 20px;border: 1px solid maroon;">
              Your details don't match any account.Check Them!!!
          </div>
          </div><?php 
        }
      }
      ...
      <div class="text-center" style="padding-top: 10px;">
        <form  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST">
          ...
          <div style="color: white;font-size: 25px;" class="slider">Security Question</div>
          <br>
          <div class="slider" >
          <select name="ques" class="drop" style="text-align: center;" >
          <option value="1">Which colour is your favourite?</option>
          <option value="2">Which book is your favourite?</option>
          <option value="3">Which anime is your favourite?</option>
          <option value="4">Which game is your favourite?</option>
          </select>
          </div>
          <br>
          <input type="text" class="slidel" name="roll" placeholder="Answer of Security Question" value="<?php echo htmlspecialchars($roll);?>">
          <br>
          <br>
          <?php if ($error["roll"]!='') { ?>
          <div class="container text-center dibba fadein" style="text-align: center;align-self: center;">
